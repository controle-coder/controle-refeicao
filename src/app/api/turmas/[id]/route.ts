...
export async function PUT(request: NextRequest, ctx: RouteContext<'/api/turmas/[id]'>) {
  try {
    await requireAdmin()
...
export async function DELETE(_req: NextRequest, ctx: RouteContext<'/api/turmas/[id]'>) {
  try {
    await requireAdmin()
    const { id } = await ctx.params
    await prisma.turma.update({ where: { id: Number(id) }, data: { ativo: false } })
